...
export default function TemplateStorageList() {
  const [templateStorages, setTemplateStorages] = useState([]);
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      fetch(`${domainurl}${address}/templateStorage`, {
        headers: {
        ...
        .then((data) => setTemplateStorages(data.storages))
        .catch((error) => {
      ...
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          ...
          <p className="mt-2 text-sm text-light-color">
            A list of all the template storages and their info about them.
          </p>
        </div>
      </div>
                ...
                {templateStorages.map((storage) => (
                  <tr key={storage}>
                    ...
                    <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-0">
                      <Link className="hover:text-blurple" href={`/templates/${storage}`}>Edit</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
